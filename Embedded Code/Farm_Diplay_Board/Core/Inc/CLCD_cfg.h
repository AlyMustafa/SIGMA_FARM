/*************************************************************/
/*************************************************************/
/***********       Author : Mohamed Ahmed      ***************/
/***********       File : CLCD_CFG.h           ***************/
/***********       Version : 1.0               ***************/
/*************************************************************/
/*************************************************************/

#ifndef CLCD_CFG_H_
#define CLCD_CFG_H_
#include "CLCD_prv.h"


/* Please choose which port to connect ctrl pins:  RS, RW, E
 * options: 1- DIO_u8PORTA
 *          2- DIO_u8PORTB
 *          3- DIO_u8PORTC
 *          4- DIO_u8PORTD
 * */
/* configure the operation mode, options: 1- EIGHT_BIT_MODE
 *                                        2- FOUR_BIT_MODE
 */
#define CLCD_OP_MODE              FOUR_BIT_MODE
/* configure if the RW pin is controlled or not, options: 1- ENABLED
 *                                                       2- DISABLED
 */
#define CLCD_RW_CRTL_EN           DISABLED

#define CLCD_CTRL_PORT            GPIOC

#define CLCD_RS_PIN               GPIO_PIN_4
#define CLCD_E_PIN                GPIO_PIN_5

#if CLCD_RW_CRTL_EN == ENABLED
#define CLCD_RW_PIN               DIO_u8PIN0
#endif

#define CLCD_DATA_PORT            GPIOB

#if CLCD_OP_MODE == FOUR_BIT_MODE
#define CLCD_D4_PIN             GPIO_PIN_0
#define CLCD_D5_PIN             GPIO_PIN_1
#define CLCD_D6_PIN             GPIO_PIN_2
#define CLCD_D7_PIN             GPIO_PIN_3
#endif


#endif
